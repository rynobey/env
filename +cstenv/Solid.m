classdef Solid < handle
  ...
  methods
    ...
    function seq = Rotate(sol, pivotX, pivotY, pivotZ, angleX, angleY, ...
      angleZ, doUpdate)
      objName = 'Transform';
      obj = sol.project.GetCOMObj(objName);
      seq = cstenv.CommandSequence(obj, objName);
      ...
      if length(sol.ports) > 0 %#ok<ISMT>
        for n = 1:length(sol.ports)
          tSeq = sol.ports(n).Rotate(pivotX, pivotY, pivotZ, angleX, angleY, ...
            angleZ, doUpdate);
        end
        seq = cstenv.CommandSequence.Join(seq, tSeq);
      end
    ...
    function seq = RotateAndCopyExpr(sol, pExprX, pExprY, pExprZ, aExprX, ...
      aExprY, aExprZ, repetitions)
      objName = 'Transform';
      obj = sol.project.GetCOMObj(objName);
      seq = cstenv.CommandSequence(obj, objName);
      seq.Add('Reset')
      ...
      if length(sol.ports) > 0 %#ok<ISMT>
        for n = 1:length(sol.ports)
          tSeq = sol.ports(n).RotateAndCopyExpr(pExprX, pExprY, pExprZ, ...
            aExprX, aExprY, aExprZ, repetitions);
        end
        seq = cstenv.CommandSequence.Join(seq, tSeq);
      end
    end
    function seq = RotateAndCopy(sol, pivotX, pivotY, pivotZ, angleX, angleY, ...
      angleZ, repetitions)
      objName = 'Transform';
      obj = sol.project.GetCOMObj(objName);
      seq = cstenv.CommandSequence(obj, objName);
      ...
      if length(sol.ports) > 0 %#ok<ISMT>
        for n = 1:length(sol.ports)
          tSeq = sol.ports(n).RotateAndCopy(pivotX, pivotY, pivotZ, angleX, ...
            angleY, angleZ, repetitions);
        end
        seq = cstenv.CommandSequence.Join(seq, tSeq);
      end
    end
  ...
  methods (Static)
    ...
    function seq = ExtRotateExpr(project, solidPath, pExprX, pExprY, pExprZ, ...
      aExprX, aExprY, aExprZ)
      objName = 'Transform';
      obj = project.GetCOMObj(objName);
      seq = cstenv.CommandSequence(obj, objName);
    ...
    function seq = ExtRotate(project, solidPath, pivotX, pivotY, pivotZ, ...
    ...
    function seq = ExtRotateAndCopyExpr(project, solidPath, pExprX, pExprY, ...
      pExprZ, aExprX, aExprY, aExprZ, repetitions)
      objName = 'Transform';
      obj = project.GetCOMObj(objName);
      seq = cstenv.CommandSequence(obj, objName);
      seq.Add('Reset')
    ...
    function seq = ExtRotateAndCopy(project, solidPath, pivotX, pivotY, ...
      pivotZ, angleX, angleY, angleZ, repetitions)
      objName = 'Transform';
      obj = project.GetCOMObj(objName);
      seq = cstenv.CommandSequence(obj, objName);
      seq.Add('Reset')
