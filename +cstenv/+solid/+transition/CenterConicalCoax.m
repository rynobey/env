classdef CenterConicalCoax < cstenv.Solid
  ...
  methods
    ...
    function seq = Create(trans)
      ...
      cSeq.cmdArr(end + 1) = cstenv.Command(obj, 'Solid', 'Subtract', ...
        trans.conicalTxLine.solidPath, c3Path);
      seq = cstenv.CommandSequence.Join(seq, cSeq);
      
      aSeq = trans.coaxialTxLine.MoveExpr('0', '0', sprintf('%s + %s/2', ...
        a.name, trans.coaxialTxLine.length.name));
