classdef TxLine < cstenv.Solid
    ...
    methods
        ...
        function seq = Create(tx)
            ...
            seq.Add('Axis', 'z');
            seq.Add('ZRange', sprintf('-%s/2', tx.length.name), sprintf('%s/2', tx.length.name));
        ...
        function seq = PickPortFace(tx, direction)
            ...
            if strcmp(direction,'+')
                seq.Add('PickFaceFromPoint', tx.solidPath, dR, num2str(0), sprintf('%s/2', tx.length.name));
            elseif strcmp(direction,'-')
                seq.Add('PickFaceFromPoint', tx.solidPath, dR, num2str(0), sprintf('-%s/2', tx.length.name));
            end
        end
        function impedance = get.impedance(tx)
            impedance = cstenv.solid.coaxial.TxLine.CalcImpedance(tx.innerRadius.value, tx.outerRadius.value);
        end
    end
    methods (Static)
