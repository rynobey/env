    ...
    methods (Static)
        function EvalInTemp(cmds)
            ...
            for n = 1:evalin('base', 'length(cmds_temp_ws);')
                ...
                clearvars n;
                eval(evalin('base', sprintf('cmds_temp_ws{%d};', evalin('base', 'n_temp_ws'))));
        ...
        function firstOrUniq = GetFirstOrUniqIndexes(resultDir)
            ...
            cd(resultDir);
            Parameters();
            cmds = {'Parameters;', 'S1 = who();', 'assignin(''caller'', ''paramList'', S1)'};
            cstenv.Results.EvalInTemp(cmds);
...
%             fOu = cstenv.Results.RemoveDuplicates(resultDir);
%             curPath = cd;
%             %get list of result files for both dirs
%             cd(sourceResultDir);
%             fileList = ls;
...
%                     %read contents of source file
%                     fid = fopen(filePath, 'a+');
%                     filePath = fullfile(resultDir, fileList(i,:));
