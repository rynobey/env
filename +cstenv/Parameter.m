  ...
  methods
      ...
      for n = 1:length(param)
        try
          if strcmp(class(param(n).value), 'double')
            param(n).value = param(n).project.CSTProject...
              .invoke('RestoreDoubleParameter', param(n).name);
          elseif ischar(class(param.value))
            param(n).value = param(n).project.CSTProject...
              .invoke('RestoreParameterExpression', param(n).name);
          end
          param(n).description = param(n).project.CSTProject...
            .invoke('GetParameterDescription', param(n).name);
        catch e
          disp(e.message)
        end
      end
    ...
    function param = Set(param)
      for n = 1:length(param)
        try
          if strcmp(class(param(n).value), 'double')
            param(n).project.CSTProject....
              .invoke('StoreDoubleParameter', param(n).name, num2str(param(n).value));
          elseif ischar(class(param(n).value))
            param(n).project.CSTProject...
              .invoke('StoreParameter', param(n).name, param(n).value);
          end
          param(n).project.CSTProject...
            .invoke('SetParameterDescription', param(n).name, param(n).description);
        catch e
          disp(e.message)
        end
      end
